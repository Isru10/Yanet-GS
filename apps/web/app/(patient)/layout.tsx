...
import {
  ...
  LogOut, Heart, Menu, X, ChevronRight, CalendarCheck,
} from "lucide-react";
...
const NAV = [
  ...
  { href: "/patient/files",     label: "My Files", icon: FolderOpen },
...
export default function PatientLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      // profiles.name (real schema uses `name` not `full_name`)
      const { data: prof } = await supabase
        .from("profiles").select("name").eq("id", user.id).single();
      if (prof) setUserName(prof.name);

      // bookings table: match by patient_email (denormalized in real schema)
    ...
    <div className="flex flex-col h-full">
      <div className="p-5 border-b border-sidebar-border">
        <Link href="/" className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-sidebar-foreground/50 text-[10px] uppercase tracking-wide mt-0.5">Patient Portal</p>
          </div>
        </Link>
      </div>
      ...
      <div className="mx-4 mt-4 p-3 bg-sidebar-accent rounded-xl flex items-center gap-3">
        <div className="w-8 h-8 bg-sidebar-primary/20 rounded-lg flex items-center justify-center">
          <CalendarCheck className="w-4 h-4 text-sidebar-primary" />
        </div>
        <div>
      ...
      <div className="p-4 border-t border-sidebar-border">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sidebar-foreground font-semibold text-sm truncate">{userName}</p>
            <p className="text-sidebar-foreground/50 text-xs">Patient</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="lg:hidden flex items-center gap-3 h-14 px-4 border-b border-border bg-white">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-emerald-900 text-sm">Yanet Patient</span>
          </div>
        </div>
